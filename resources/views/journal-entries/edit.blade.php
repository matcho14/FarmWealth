...
<div class="card">
    <div class="card-header bg-primary text-white">
        <h5 class="mb-0"><i class="fas fa-exchange-alt me-2"></i>تعديل قيد محاسبي يدوي رقم {{ $journalEntry->entry_number }}</h5>
    </div>
    <div class="card-body">
        @if($errors->has('lines'))
        ...
        <form action="{{ route('journal-entries.update', $journalEntry) }}" method="POST">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control bg-light" value="{{ $journalEntry->entry_number }}" readonly>
                </div>
                <div class="col-md-3">
                    <label class="form-label">تاريخ القيد <span class="text-danger">*</span></label>
                    <input type="date" name="entry_date" class="form-control" value="{{ old('entry_date', $journalEntry->entry_date->toDateString()) }}" required>
                </div>
                <div class="col-md-6">
                            ...
                            <tr class="line-row">
                                 <td>
                                     <select name="lines[{{ $i }}][account_type]" class="form-select form-select-sm account-type" required>
                                         <option value="treasury" {{ $line->account_type === 'treasury' ? 'selected' : '' }}>خزنة</option>
                                         <option value="supplier" {{ $line->account_type === 'supplier' ? 'selected' : '' }}>مورد</option>
                                         <option value="client" {{ $line->account_type === 'client' ? 'selected' : '' }}>عميل</option>
                                         <option value="chart_of_account" {{ $line->account_type === 'chart_of_account' ? 'selected' : '' }}>حساب من شجرة الحسابات</option>
                                         <option value="cycle" {{ $line->account_type === 'cycle' ? 'selected' : '' }}>دورة</option>
                                         <option value="sales" {{ $line->account_type === 'sales' ? 'selected' : '' }}>مبيعات</option>
                                     </select>
                                 </td>
                                 <td>
                                     <select name="lines[{{ $i }}][account_id]" class="form-select form-select-sm account-id" required>
                                         @foreach($treasuries as $t)
                                             <option value="{{ $t->id }}" data-type="treasury" {{ $line->account_type === 'treasury' && old('lines.'.$i.'.account_id', $line->account_id) == $t->id ? 'selected' : '' }}>{{ $t->name }}</option>
                                         @endforeach
                                         @foreach($suppliers as $s)
                                             <option value="{{ $s->id }}" data-type="supplier" {{ $line->account_type === 'supplier' && old('lines.'.$i.'.account_id', $line->account_id) == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                                         @endforeach
                                         @foreach($clients as $c)
                                             <option value="{{ $c->id }}" data-type="client" {{ $line->account_type === 'client' && old('lines.'.$i.'.account_id', $line->account_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                                         @endforeach
                                         @foreach($chartAccounts as $acc)
                                             <option value="{{ $acc->id }}" data-type="chart_of_account" {{ $line->account_type === 'chart_of_account' && old('lines.'.$i.'.account_id', $line->account_id) == $acc->id ? 'selected' : '' }}>{{ $acc->full_code }} - {{ $acc->name }}</option>
                                         @endforeach
                                     </select>
                                 </td>
                                <td><input type="number" name="lines[{{ $i }}][debit]" class="form-control form-control-sm debit-input" min="0" step="0.01" value="{{ old('lines.'.$i.'.debit', $line->debit) }}" oninput="calcTotals()"></td>
                                <td><input type="number" name="lines[{{ $i }}][credit]" class="form-control form-control-sm credit-input" min="0" step="0.01" value="{{ old('lines.'.$i.'.credit', $line->credit) }}" oninput="calcTotals()"></td>
                                <td><input type="text" name="lines[{{ $i }}][description]" class="form-control form-control-sm" value="{{ old('lines.'.$i.'.description', $line->description) }}" placeholder="بيان"></td>
            ...
            <div class="mb-3">
                ...
                <textarea name="notes" class="form-control" rows="2">{{ old('notes', $journalEntry->notes) }}</textarea>
            </div>

            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-success btn-lg flex-grow-1"><i class="fas fa-save me-2"></i>تحديث القيد</button>
...
@section('scripts')
<script>
let lineIdx = {{ $journalEntry->lines->count() }};
