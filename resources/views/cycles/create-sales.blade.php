    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">إضافة مبيعات للدورة #{{ $cycle->id }}</h5>
            </div>
            <div class="card-body">
                <div class="alert alert-info mb-4">
                    الكتاكيت المتبقية المتاحة للبيع: <strong>{{ $cycle->expected_remaining }}</strong>
                </div>

                <form action="{{ route('cycles.storeSales', $cycle) }}" method="POST">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('payment_type') is-invalid @enderror" id="payment_type" name="payment_type" required onchange="togglePaymentFields()">
                                ...
                                <option value="cash" {{ old('payment_type') == 'cash' ? 'selected' : '' }}>كاش (تحصيل فوري للخزنة)</option>
                                <option value="credit" {{ old('payment_type') == 'credit' ? 'selected' : '' }}>إجل (سداد لاحق)</option>
                            </select>
                            @error('payment_type')
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="number" class="form-control @error('sold_count') is-invalid @enderror" id="sold_count" name="sold_count" value="{{ old('sold_count') }}" min="1" max="{{ $cycle->expected_remaining }}" required>
                            @error('sold_count')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" class="form-control @error('weight') is-invalid @enderror" id="weight" name="weight" value="{{ old('weight') }}" min="0.1" required>
                            @error('weight')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-12">
                            ...
                            <input type="text" class="form-control @error('description') is-invalid @enderror" id="description" name="description" value="{{ old('description') }}" placeholder="مثال: بيع للعميل أحمد">
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-2"></i>حفظ المبيعات
                        </button>
