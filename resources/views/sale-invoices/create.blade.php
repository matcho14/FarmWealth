...
<div class="card">
    <div class="card-header bg-primary text-white">
        <h5 class="mb-0"><i class="fas fa-file-invoice me-2"></i>إنشاء فاتورة بيع</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('sale-invoices.store') }}" method="POST" id="invoiceForm">
            @csrf
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    ...
                    <input type="hidden" name="invoice_number" value="{{ $nextNumber }}">
                </div>
                <div class="col-md-3">
                                ...
                                <td>
                                    <select name="items[{{ $i }}][item_id]" class="form-select form-select-sm item-select" required>
                                        ...
                                        @foreach($items as $item)
                                            <option value="{{ $item->id }}" data-price="{{ $item->last_purchase_price ?? 0 }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td><input type="number" name="items[{{ $i }}][quantity]" class="form-control form-control-sm quantity-input" min="0.001" step="0.001" value="1" oninput="calcItemTotal(this)"></td>
                                <td><input type="number" name="items[{{ $i }}][unit_price]" class="form-control form-control-sm price-input" min="0" step="0.01" value="0" oninput="calcItemTotal(this)"></td>
                                <td class="item-total">0</td>
                                <td class="text-center"><button type="button" class="btn btn-sm btn-danger remove-item" style="display:none"><i class="fas fa-times"></i></button></td>
                            </tr>
                            @endfor
                        </tbody>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <input type="number" name="paid_amount" id="paidAmount" class="form-control" min="0" step="0.01" value="0" oninput="updatePaymentStatus()">
                </div>
                <div class="col-md-6">
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-success btn-lg flex-grow-1"><i class="fas fa-save me-2"></i>حفظ الفاتورة</button>
