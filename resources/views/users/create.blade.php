            ...
            <div class="card-body">
                <form action="{{ route('users.store') }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-bold">كلمة السر <span class="text-danger">*</span></label>
                            <input type="password" name="password" class="form-control @error('password') is-invalid @enderror" required>
                            @error('password') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-bold">تأكيد كلمة السر <span class="text-danger">*</span></label>
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="role" id="roleSelect" class="form-select" required onchange="togglePermissions()">
                                <option value="user" {{ old('role') == 'user' ? 'selected' : '' }}>مستخدم عادي (صلاحيات محددة)</option>
                                <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>مدير نظام (Admin - تحكم كامل)</option>
                            </select>
                        </div>
                    </div>

                    <div id="permissionsSection" class="mt-4 col-md-12" style="{{ old('role') == 'admin' ? 'display:none' : '' }}">
                        <h6 class="fw-bold border-bottom pb-2 mb-3">تحديد الصلاحيات (القوائم المسموح بها)</h6>
                        <div id="shedAssignment" class="col-md-12 mb-3" style="{{ (old('permissions') && in_array('sheds', old('permissions'))) ? '' : 'display:none' }}">
                            ...
                            <select name="assigned_shed_id" class="form-select">
                                ...
                                @foreach($sheds as $shed)
                                    <option value="{{ $shed->id }}" {{ old('assigned_shed_id') == $shed->id ? 'selected' : '' }}>
                                        {{ $shed->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                <div class="form-check card p-2 h-100 hover-shadow transition-all">
                                    <input class="form-check-input ms-2" type="checkbox" name="permissions[]" value="{{ $key }}" id="perm_{{ $key }}" {{ is_array(old('permissions')) && in_array($key, old('permissions')) ? 'checked' : '' }}>
